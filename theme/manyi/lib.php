 ...
 * the {@link theme_manyi_less_variables()} here because we need to create selectors or
 * alter existing ones.
 *
 * @param theme_config $theme The theme config object.
 * @return string Raw LESS code.
 */
function theme_manyi_extra_less($theme) {
    $content = '';
 ...
 */
function theme_manyi_process_css($css, $theme) {
    global $OUTPUT;

    // Set the background image for the logo.
    $logo = $OUTPUT->get_logo_url(null, 100);
    $css = theme_manyi_set_logo($css, $logo);
    ...
    $css = theme_manyi_set_customcss($css, $customcss);
 ...
 */
function theme_manyi_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_manyi_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM &&
            ($filearea === 'logo' || $filearea === 'smalllogo' || $filearea === 'backgroundimage')) {
        $theme = theme_config::load('manyi');
 ...
 */
function theme_manyi_set_customcss($css, $customcss) {
...
function theme_manyi_less_variables($theme) {
    $variables = array();
    ...
    if (!empty($theme->settings->h2color)) {
        $variables['h2color'] =
        $theme->settings->h2color;
    }
    if (!empty($theme->settings->phover)) {
        $variables['phover'] =
        $theme->settings->phover;
    }
    if (!empty($theme->settings->fontsize)) {
        $variables['fontsize'] =
        $theme->settings->fontsize;
    }
    if (!empty($theme->settings->file)) {
        $variables['file'] =
        $theme->settings->file;
    }
    return $variables;
